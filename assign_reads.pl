...
open (NOTSURE, ">$file_notsure") || die "MainError: write to $file_notsure\n";
foreach my $readid (keys %include) {
	if (exists $exclude{$readid}) {
		print NOTSURE $readid."\n";
		delete $exclude{$readid};
	}
	else {
		print INCLUDE $readid."\n";
	}
}
close INCLUDE;
close NOTSURE;
